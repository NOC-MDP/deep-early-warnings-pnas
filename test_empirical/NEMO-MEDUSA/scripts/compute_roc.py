...
def compute_roc(parameter:str,region:str,bool_pred_early=False,five_hundred = False):
    os.makedirs(f"{region}/results/{parameter}/roc/1500", exist_ok=True)
    os.makedirs(f"{region}/results/{parameter}/roc/500", exist_ok=True)
    # --------
    ...
    df_ews_null = pd.read_csv(f"{region}/results/{parameter}/ews/df_ews_null.csv")

    # Import kendall tau data
    df_ktau_forced = pd.read_csv(f"{region}/results/{parameter}/ews/df_ktau_forced.csv")
    df_ktau_null = pd.read_csv(f"{region}/results/{parameter}/ews/df_ktau_null.csv")

    if five_hundred:
        # Import ML predictions
        df_ml_forced = pd.read_csv(f"{region}/results/{parameter}/ml_preds/500/parsed/df_ml_forced.csv")
        df_ml_null = pd.read_csv(f"{region}/results/{parameter}/ml_preds/500/parsed/df_ml_null.csv")
    else:
        # Import ML predictions
        df_ml_forced = pd.read_csv(f"{region}/results/{parameter}/ml_preds/1500/parsed/df_ml_forced.csv")
        df_ml_null = pd.read_csv(f"{region}/results/{parameter}/ml_preds/1500/parsed/df_ml_null.csv")
    ...
    export_path = f"{region}/results/{parameter}/"
    if five_hundred:
