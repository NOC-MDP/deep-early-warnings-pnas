...
# --------------
def dl_apply_len1500(parameter:str,tsid_vals,region:str):
    os.makedirs(f"{region}/results/{parameter}/ml_preds/1500/", exist_ok=True)
    ...
    for tsid in tsid_vals:
        ...
        dl_preds_mean.to_csv(
            f"{region}/results/{parameter}/ml_preds/1500/ensemble_trend_probs_dakos_forced_{tsid}.csv",
        )
    ...
    for tsid in tsid_vals:
        for null_number in null_numbers:
            ...
            # Export
            dl_preds_mean.to_csv(
                f"{region}/results/{parameter}/ml_preds/1500/ensemble_trend_probs_dakos_null_{tsid}_{null_number}.csv",
            )

if __name__ == "__main__":
    dl_apply_len1500(parameter="CHL",tsid_vals=[1,2,3],region="Labrador_Sea")
