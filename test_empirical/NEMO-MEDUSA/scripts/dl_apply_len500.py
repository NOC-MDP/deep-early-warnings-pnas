...
def dl_apply_len500(tsid_vals,parameter:str,region:str):
    ...
    for tsid in tsid_vals:
        ...
        dl_preds_mean.to_csv(
            f"{region}/results/{parameter}/ml_preds/500/ensemble_trend_probs_dakos_forced_{tsid}.csv",
        )
    ...
    for tsid in tsid_vals:
        for null_number in null_numbers:
            ...
            # Export
            dl_preds_mean.to_csv(
                f"{region}/results/{parameter}/ml_preds/500/ensemble_trend_probs_dakos_null_{tsid}_{null_number}.csv",
            )
